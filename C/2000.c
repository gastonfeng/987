...
bit flag,ff;
bit pflag,round;
	code unsigned char *sprompt[]={"蓄电池电压系数：","辐射量 2 系数：","温度计算系数：","辐射量 1 系数：","放电电流系数：","充电电流 2 系数：","充电电流 1 系数："};
...
void int0(void) interrupt 0 using 2			/*	硬时钟每秒中断一次	*/
{
	unsigned char ch;
...
/*				RecordDate[PointDate].WindCurrentMaximum=Maximum[3];
				...
				RecordMonth[PointMonth].Radio1Total+=RecordDate[PointDate].Radio1Total;	/*	月累计	*/
				...
				switch(wait_key()){
					...
					case OK:{*/
						while(1){
							...
							gotoxy(2,0);
							pprintf("辐射量1:%d 2:%dkW",RecordYear[year].RadioTotal,RecordYear[year].Radio1Total);
							gotoxy(4,0);
							pprintf("充电量1:%d 2:%dkAH",RecordYear[year].ChargeTotal,RecordYear[year].Charge1Total);
							gotoxy(6,0);
							pprintf("放电量:%dkAH",RecordYear[year].DischargeTotal);
							switch(wait_key()){
								case INC:year=(year>=PointYear)?0:year+1;break;
								case DEC:year=(year<=0)?PointYear:year-1;break;
								default:return;
							...
							switch(wait_key()){
								...
								case OK:{*/
									while(1){
										...
										gotoxy(2,0);
										pprintf("辐射量1:%d 2:%dW",RecordMonth[i].RadioTotal,RecordMonth[i].Radio1Total);
										gotoxy(4,0);
										pprintf("充电量1:%d 2:%dAH",RecordMonth[i].ChargeTotal,RecordMonth[i].Charge1Total);
										gotoxy(6,0);
										pprintf("放电量:%dAH",RecordMonth[i].DischargeTotal);
										switch(wait_key()){
...
void printhis(void)
{
	...
	while(1){                                   
		...
		pprintf("时间\t辐射量1\t辐射量2\t蓄电池电压\t充电电流1\t充电电流2\t放电电流\t温度\n\r");
		for(h=0;h<24;h++)
		...
		pprintf("当天最大值\t%dW\t%dW\t%oV\t%oA\t%oA\t%oA\t%o℃\n\r",
		...
		pprintf("当天最小值\t%dW\t%dW\t%oV\t%oA\t%oA\t%oA\t%o℃\n\r",
		...
		pprintf("充电1电量:%oAh\t充电2电量:%oAh\t放电电量:%oAh\n\r\t辐射量1:%dW\t辐射量2:%dW\n\r",
            ...
            if(mflag){
                ...
                switch(m.frame){
                    ...
                    case 5:{
                    	/*clrscr();*/
                    	pprintf("辐射量 1:     ");
                    	pprintf("\n\n当前值:%dW/m    ",getresult(3));
                    	/*pprintf("\n\n%s%dW/m",msg17,Average[3]);*/
                    	pprintf("\n\n今日最大值:%dW/m    \n\n今日最小值:%dW/m    ",Maximum[3],Minimum[3]);
                    	break;
                    }
                    case 6:{
                    	/*clrscr();*/
                    	pprintf("辐射量 2:     ");
                    	pprintf("\n\n当前值:%dW/m    ",getresult(1));
                    	/*pprintf("\n\n%s%dW/m",msg17,Average[3]);*/
