...
typedef struct{
	...
	unsigned char month;
	unsigned char date;
	unsigned int BatteryVoltageAverage[24],RadioAverage[24],ChargeCurrentAverage[24],DischargeCurrentAverage[24],WindCurrentAverage[24];
	int TemperatureMaximum,TemperatureMinimum,TemperatureAverage[24];
	unsigned int BatteryVoltageMaximum,BatteryVoltageMinimum,RadioMaximum,RadioMinimum,ChargeCurrentMaximum,ChargeCurrentMinimum,DischargeCurrentMaximum,DischargeCurrentMinimum,WindCurrentMaximum,WindCurrentMinimum;
...
#define Used		(*(unsigned char *)0x020000)	/*	数据区使用标志,占用1字节	*/
#define PointDate 	(*(unsigned char *)0x020001)	/*	天数据指针,占用一字节		*/
#define PointMonth	(*(unsigned char *)0x020002)	/*	月数据指针,占用一字节		*/
#define PointYear	(*(unsigned char *)0x020003)	/*	年数据指针,占用一字节		*/
#define RecordDates	(*(unsigned char *)0x020004)	/*	天数据记录数,占用一字节		*/
#define RecordMonths	(*(unsigned char *)0x020005)	/*	月数据记录数,占用一字节		*/
#define RecordYears	(*(unsigned char *)0x020006)	/*	年数据记录数,占用一字节		*/
#define Contrast	(*(unsigned char *)0x020007)	/*	液晶显示器对比度,占用一字节	*/
#define Scale		((float *)0x020008)		/*	模拟量输入转换系数,占用4字节*6通道=24字节	*/
#define RecordDate	((recorddate *)0x020020)	/*	天数据记录区,占用31566字节	*/
#define RecordMonth	((recordmonth *)0x027b64)	/*	月数据记录区,占用1080字节	*/
#define RecordYear	((recordyear *)0x027f9c)	/*	年数据记录区,占用80字节		*/
...
bit flag,ff;
bit pflag,round;
	code unsigned char *sprompt[]={"蓄电池电压系数：","辐射量系数：","温度计算系数：","风机充电电流系数","放电电流系数：","充电电流系数："};
	code float ScaleDefault[6]={0.5273,1.6608,1,0.6104,0.6104,0.6104};
	bit sflag;
...
void int0() interrupt 0			/*	硬时钟每秒中断一次	*/
{
	...
	if(Secondss==0){	/*	整秒,每分钟操作一次,A/D采样并比较最大最小值	*/
		...
		if(Minutess==0){	/*	整分,每小时一次,计算记录每小时平均值	*/
			...
			RecordDate[PointDate].ChargeTotal+=Average[5]+Average[3];
												...
												while(1){
													...
													while(1){
														...
														if(frame<8){
															...
															gotoxy(7,0);
															pprintf("Wind: %o %o %oA",RecordDate[i].WindCurrentAverage[frame*3],RecordDate[i].WindCurrentAverage[frame*3+1],RecordDate[i].WindCurrentAverage[frame*3+2]);
														}
														else{
															...
															gotoxy(6,0);
															pprintf("Wind: %o %oA",RecordDate[i].WindCurrentMaximum,RecordDate[i].WindCurrentMinimum);
														}
														Asciibold=1;
														switch(wait_key()){
															case INC:i=(i>=((start==0)?PointDate:97))?0:i+1;break;
															case DEC:i=(i<=0)?((start==0)?PointDate:97):i-1;break;
...
void printhis(void)
{
	...
	Prn=1;
	pflag=1;
	if(RecordDate[97].year==0)round=0;else round=1;
	...
	while(1){
		...
		pprintf("时间\t辐射量\t蓄电池电压\t充电电流\t放电电流\t温度\t风机电流\n\r");
		for(h=0;h<24;h++)pprintf("%b:00\t%dW\t%oV\t%oA\t%oA\t%o℃\t%oA\n\r",h+1,RecordDate[i].RadioAverage[h],RecordDate[i].BatteryVoltageAverage[h],RecordDate[i].ChargeCurrentAverage[h],RecordDate[i].DischargeCurrentAverage[h],RecordDate[i].TemperatureAverage[h]-200,RecordDate[i].WindCurrentAverage[h]);
		pprintf("当天最大值\t%dW\t%oV\t%oA\t%oA\t%o℃\t%oA\n\r",RecordDate[i].RadioMaximum,RecordDate[i].BatteryVoltageMaximum,RecordDate[i].ChargeCurrentMaximum,RecordDate[i].DischargeCurrentMaximum,RecordDate[i].TemperatureMaximum-200,RecordDate[i].WindCurrentMaximum);
		pprintf("当天最小值\t%dW\t%oV\t%oA\t%oA\t%o℃\t%oA\n\r\n\r",RecordDate[i].RadioMinimum,RecordDate[i].BatteryVoltageMinimum,RecordDate[i].ChargeCurrentMinimum,RecordDate[i].DischargeCurrentMinimum,RecordDate[i].TemperatureMinimum-200,RecordDate[i].WindCurrentMinimum);
    ...
    while(1){
        ...
        if(mdisplay){
            switch(getch()){
                ...
                case INC:mflag=1;m.frame++;if(m.frame==7)m.frame=0;clrscr();keypress=1;break;
                case DEC:mflag=1;if(m.frame==0)m.frame=6;else m.frame--;clrscr();keypress=1;break;
