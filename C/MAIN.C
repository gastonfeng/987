...
bit flag,ff;
bit pflag,round;
	code unsigned char *sprompt[]={"蓄电池电压系数：","辐射量系数：","温度计算系数：","放电电流系数：","充电电流系数："};
	code float ScaleDefault[5]={0.3516,1.4648,1,0.4883,0.4883};
	bit sflag;

bit intflag,mflag,mdisplay,Asciibold;    /*  中断标志    */
bit Prn,Prtbytimeflag,Device;
unsigned int time;
...
unsigned int memory[5],loop[5],Maximum[5],Minimum[5],Current[5],Average[5];
...
void pprintf (unsigned char *format,...)
{
	...
	unsigned int div_val,  basenumber;
	int u_val;
    va_list ap;
    va_start(ap,format);
    cc=0;
    for (;;){
        ...
        switch (format_flag = *format++){
            ...
            case 'b':
            	...
            	goto l1;
            case 'f':
            	u_val=va_arg(ap,float)*10000;
				basenumber=10;
				pputchar(u_val/10000+48);
				pputchar('.');         
				u_val%=10000;
				div_val=1000;
				do{
					pputchar(u_val/div_val+48);
					u_val%=div_val;
					div_val/=basenumber;
				}while(div_val);
				continue;  	
...
void int0() interrupt 0
{
	...
	if(Secondss==0){
		...
		if(Minutess==0){
			...
			RecordDate[PointDate].ChargeCurrentAverage[Hours]=Average[4];
			RecordDate[PointDate].DischargeCurrentAverage[Hours]=Average[3];
			RecordDate[PointDate].TemperatureAverage[Hours]=Average[2];
			RecordDate[PointDate].ChargeTotal+=Average[4];
			RecordDate[PointDate].DischargeTotal+=Average[3];
			RecordDate[PointDate].RadioTotal+=Average[1];
			loop[0]=loop[1]=loop[2]=loop[3]=loop[4]=0;
			memory[0]=memory[1]=memory[2]=memory[3]=memory[4]=0;
			if(Hourss==0){
				...
				RecordDate[PointDate].ChargeCurrentMaximum=Maximum[4];
				RecordDate[PointDate].ChargeCurrentMinimum=Minimum[4];
				RecordDate[PointDate].DischargeCurrentMaximum=Maximum[3];
				RecordDate[PointDate].DischargeCurrentMinimum=Minimum[3];
				RecordDate[PointDate].TemperatureMaximum=Maximum[2];
				RecordDate[PointDate].TemperatureMinimum=Minimum[2];
				RecordMonth[PointMonth].RadioTotal+=RecordDate[PointDate].RadioTotal;
				RecordMonth[PointMonth].ChargeTotal+=RecordDate[PointDate].ChargeTotal;
				RecordMonth[PointMonth].DischargeTotal+=RecordDate[PointDate].DischargeTotal;
				PointDate++;
				RecordDate[PointDate].year=Years+1980;
				RecordDate[PointDate].month=Months;
				RecordDate[PointDate].date=Dates;
				if(Dates==1){
					RecordYear[PointYear].RadioTotal+=RecordMonth[PointMonth].RadioTotal;
					RecordYear[PointYear].ChargeTotal+=RecordMonth[PointMonth].ChargeTotal;
					RecordYear[PointYear].DischargeTotal+=RecordMonth[PointMonth].DischargeTotal;
					PointMonth++;
				...
				switch(wait_key()){
					...
					case OK:{*/
						while(1){
							...
							pprintf("\n\n辐射量:%d",RecordYear[year].RadioTotal);
							pprintf("\n\n充电电量:%d",RecordYear[year].ChargeTotal);
							pprintf("\n\n放电电量:%d",RecordYear[year].DischargeTotal);
							switch(wait_key()){
								case INC:year=(year>=PointYear)?0:year+1;break;
								case DEC:year=(year<=0)?PointYear:year-1;break;
								default:return;
							...
							switch(wait_key()){
								...
								case OK:{*/
									while(1){
										...
										pprintf("\n\n辐射量:%d",RecordMonth[i].RadioTotal);
										pprintf("\n\n充电电量:%d",RecordMonth[i].ChargeTotal);
										pprintf("\n\n放电电量:%d",RecordMonth[i].DischargeTotal);
										switch(wait_key()){
												...
												while(1){
													...
													while(1){
														...
														if(frame<8){
															...
															gotoxy(2,0);
															pprintf("Battery:%oV %oV %oV",RecordDate[i].BatteryVoltageAverage[frame*3],RecordDate[i].BatteryVoltageAverage[frame*3+1],RecordDate[i].BatteryVoltageAverage[frame*3+2]);
															gotoxy(3,0);
															pprintf("Radio:  %dW %dW %dW",RecordDate[i].RadioAverage[frame*3],RecordDate[i].RadioAverage[frame*3+1],RecordDate[i].RadioAverage[frame*3+2]);
															gotoxy(4,0);
															pprintf("Charge: %oA %oA %oA",RecordDate[i].ChargeCurrentAverage[frame*3],RecordDate[i].ChargeCurrentAverage[frame*3+1],RecordDate[i].ChargeCurrentAverage[frame*3+1]);
															gotoxy(5,0);
															pprintf("Dischar:%oA %oA %oA",RecordDate[i].DischargeCurrentAverage[frame*3],RecordDate[i].DischargeCurrentAverage[frame*3+1],RecordDate[i].DischargeCurrentAverage[frame*3+2]);
															gotoxy(6,0);
															pprintf("Tempera:%o %o %o",RecordDate[i].TemperatureAverage[frame*3]-200,RecordDate[i].TemperatureAverage[frame*3+1]-200,RecordDate[i].TemperatureAverage[frame*3+2]-200);
														}
														else{
															pprintf("%b-%b-%d Maximum&Minimum",RecordDate[i].month,RecordDate[i].date,RecordDate[i].year);
															gotoxy(1,0);
															pprintf("Battery:%oV %oV",RecordDate[i].BatteryVoltageMaximum,RecordDate[i].BatteryVoltageMinimum);
															gotoxy(2,0);
															pprintf("Radio:  %dW %dW",RecordDate[i].RadioMaximum,RecordDate[i].RadioMinimum);
															gotoxy(3,0);
															pprintf("Charge: %oA %oA",RecordDate[i].ChargeCurrentMaximum,RecordDate[i].ChargeCurrentMinimum);
															gotoxy(4,0);
															pprintf("Dischar:%oA %oA",RecordDate[i].DischargeCurrentMaximum,RecordDate[i].DischargeCurrentMinimum);
															gotoxy(5,0);
															pprintf("Tempera:%o %o",RecordDate[i].TemperatureMaximum-200,RecordDate[i].TemperatureMinimum-200);
														}
														Asciibold=1;
														switch(wait_key()){
		...
		switch(wait_key()){
			...
			case DEC:{
				if(round){
					...
					else end=(end==start)?PointDate:end--;
				}
				else end=(end==start)?PointDate:end--;
				break;
			}
    ...
    while(1){
        ...
        if(mdisplay){
            switch(getch()){
                ...
                case INC:mflag=1;m.frame++;if(m.frame==6)m.frame=0;clrscr();break;
                case DEC:mflag=1;if(m.frame==0)m.frame=5;else m.frame--;clrscr();break;
